    ...
    def list_attributes(self, *args, **kwargs):
        ...
        try:
            ...
            query = FIADBreftable().get(
                tableName="REF_POP_ATTRIBUTE",
                colList="ATTRIBUTE_NBR, ATTRIBUTE_DESCR, TIMBERLAND",
            )

            self.attributes = sorted(
                [
                    (p["ATTRIBUTE_NBR"], p["ATTRIBUTE_DESCR"], p["TIMBERLAND"])
                    for p in query
                ],
...
class FIADBevalgrp(Client):
    ...
    def query(self, whereClause="", mostRecent="Y", **kwargs):
        ...
        params = {
            ...
            "mostRecent": test_param(
                mostRecent, ["Y", "N"]
            ),  # If "Y" then only the most recent inventories (based on `DATAMART_MOST_RECENT_INV`) will be returned
        }
        ...
        if resp.status_code == 200:
            ...
            raise APIException(
                "An error occured. For a list of available POP_EVAL_GRP fields to use in whereClause, evaluate `self.evalgrp.available_cols`"
            )
...
class FIADBreftable(Client):
    ...
    def tables(self, *args, **kwargs):
        ...
        if resp.status_code == 200:
            ...
            raise APIException(
                "An unknown error occured while attempting to access a list of available tables."
            )
    ...
    def query(self, tableName, colList="*", whereStr="1=1", **kwargs):
        ...
        if resp.status_code == 200:
            ...
            if "invalid identifier" in resp.text:
                raise APIException(
                    "Error " + str(resp.status_code) + ": invalid identifier"
                )
            if "invalid relational operator" in resp.text:
                raise APIException(
                    "Error "
                    + str(resp.status_code)
                    + ": invalid relational operator: use '=', '!=', 'LIKE', 'NOT LIKE', etc."
                )
            if "missing expression" in resp.text:
                raise APIException(
                    "Error "
                    + str(resp.status_code)
                    + ": missing expression (did you use '==' when you meant '='?)"
                )
            else:
                raise APIException(
                    "Error " + str(resp.status_code) + ": Unknown error occured."
                )
