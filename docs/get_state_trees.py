...
acres = { c["content"][0:5] : c["column"][0]["cellValueNumerator"] for c in (acres_per_county[1:]) }
...
data = {page["content"]: {col["content"][0:5] : toFloat( col["column"][0]["cellValueNumerator"] ) for col in (page["row"][1:]) } for page in trees_per_county[1:]}
...
for page in data:
  ...
  with open("data/"+STATE_PREFIX+"_"+title+".csv", "w+", newline='') as output_csv:
